...
#cache fields: ServiceID, Role, EthAddr, Port, TTL, Auth
...
if choice == '1': #format: serviceID role port
    ...
    if cache.has_key((sid, role)):
        ...
        os.write(pout, '1\n')
        cache[(sid, role)] = (eth, port, ttl, 'YES')
    ...
    port = int(fields[3])
    ttl = int(fields[4])
    cache[(serviceID, role)] = (eth, port, ttl, auth)
    os.write(pout, '1\n')
    ...
    if cache.has_key((serviceID, role)):
        ...
        ttl = val[2]
        auth = val[3]
        os.write(pout, eth+" "+str(port)+" "+str(ttl)+" "+auth+"\n")
    else:
        os.write(pout, '0\n') #key not found
